...
public class PersonDaoHibernate extends UniversalDaoHibernate implements PersonDao
{
	...
	public void addPersonIdentifier(PersonIdentifier identifier) {
		...
		getHibernateTemplate().flush();
		if (log.isDebugEnabled()) {
			log.debug("Finished saving identifier.");
		}
	}
...
//							log.debug("Old ID is implicitly voided: " + id);
	...
	public List<Person> getPersonsByIdentifier(final PersonIdentifier personIdentifier) {
		return (List<Person>) getHibernateTemplate().execute(new HibernateCallback<List<Person>>() {
	...
	public Person loadPerson(final Integer personId) {
		return (Person) getHibernateTemplate().execute(new HibernateCallback<Person>() {
	...
	public List<Record> loadRecords(final List<Long> recordIds) {
		return (List<Record>) getHibernateTemplate().executeFind(new HibernateCallback<List<Record>>() {
	...
	public Record loadRecord(final Long recordId) {
		return (Record) getHibernateTemplate().execute(new HibernateCallback<Record>() {
			public Record doInHibernate(Session session) throws HibernateException, SQLException {
				@SuppressWarnings("unchecked")
				List<Person> persons = session.createCriteria(Person.class)
	...
	public Boolean getPersonByIdHQL(final PersonIdentifier personIdentifier) {
		return (Boolean) getHibernateTemplate().execute(new HibernateCallback<Boolean>() {
	...
	public List<Person> getPersonsPaged(final int firstResult, final int maxResults) {
		@SuppressWarnings("unchecked")
		List<Person> persons = (List<Person>) getHibernateTemplate().execute(new HibernateCallback<List<Person>>() {
			public List<Person> doInHibernate(Session session) throws HibernateException, SQLException {
...
//	}
	...
	public IdentifierDomain findIdentifierDomainByName(final String identifierDomainName) {
		return (IdentifierDomain) getHibernateTemplate().execute(new HibernateCallback<IdentifierDomain>() {
			@SuppressWarnings("unchecked")
			public IdentifierDomain doInHibernate(Session session) throws HibernateException, SQLException {
...
*/
	...
	public List<IdentifierUpdateEvent> getIdentifierUpdateEvents(final int startIndex, final int maxEvents,
		...
		List<IdentifierUpdateEvent> identifierUpdateEvents = (List<IdentifierUpdateEvent>) getHibernateTemplate()
				.execute(new HibernateCallback<List<IdentifierUpdateEvent>>() {
					public List<IdentifierUpdateEvent> doInHibernate(Session session) throws HibernateException, SQLException {
						Query query = session
	...
	public List<IdentifierUpdateEvent> getIdentifierUpdateEvents(final User eventRecipient) {
		...
		List<IdentifierUpdateEvent> identifierUpdateEvents = (List<IdentifierUpdateEvent>) getHibernateTemplate()
				.execute(new HibernateCallback<List<IdentifierUpdateEvent>>() {
					public List<IdentifierUpdateEvent> doInHibernate(Session session) throws HibernateException,
							SQLException {
						Query query = session
								.createQuery("from IdentifierUpdateEvent i where i.updateRecipient = :updateRecipient order by i.identifierUpdateEventId");
						query.setParameter("updateRecipient", eventRecipient);
	...
	public List<IdentifierUpdateEvent> getIdentifierUpdateEventsBeforeDate(final Date startDate,
		...
		List<IdentifierUpdateEvent> identifierUpdateEvents = (List<IdentifierUpdateEvent>) getHibernateTemplate()
				.execute(new HibernateCallback<List<IdentifierUpdateEvent>>() {
					public List<IdentifierUpdateEvent> doInHibernate(Session session) throws HibernateException,
							SQLException {
						Query query = session.createQuery("from IdentifierUpdateEvent i "
								+ " where i.dateCreated < :startDate and i.updateRecipient = :updateRecipient "
								+ "order by i.dateCreated");
						query.setParameter("startDate", startDate);
						query.setParameter("updateRecipient", eventRecipient);
						log.debug("Querying using " + query.toString());
