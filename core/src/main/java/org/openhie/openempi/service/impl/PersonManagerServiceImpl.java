...
public class PersonManagerServiceImpl extends PersonCommonServiceImpl implements PersonManagerService
{
	...
	public Person addPerson(Person person) throws ApplicationException {
		...
		Context.notifyObserver(ObservationEventType.RECORD_ADD_EVENT, ConvertUtil.getRecordFromPerson(person));
	...
	public void deletePerson(PersonIdentifier personIdentifier) throws ApplicationException {
		...
		// Generate a notification event to inform interested listeners via the lightweight mechanism that this event has occurred.
		Context.notifyObserver(ObservationEventType.RECORD_DELETE_EVENT, ConvertUtil.getRecordFromPerson(personFound));
	}
	...
	public void mergePersons(PersonIdentifier retiredIdentifier, PersonIdentifier survivingIdentifier) throws ApplicationException {
		...
		PersonLink exists = personLinkDao.getPersonLink(personSurviving, personRetiring);
		if (exists != null) {
			log.warn("Attempted to merge records " + personSurviving.getPersonId() + " and " +
					personRetiring.getPersonId() + " which are already linked.");
			throw new ApplicationException("Person records to be merged are already linked; you may want to delete the retiring person record instead.");
		}
		
		List<PersonLink> origLinks = personLinkDao.getPersonLinks(personRetiring);
		List<PersonLink> survLinks = personLinkDao.getPersonLinks(personSurviving);
		for (PersonLink originalLink : origLinks) {
			...
			synchronizeGlobalIdentifiers(personSurviving, otherPerson);
			for (PersonLink survLink : survLinks) {
				Person survOtherPerson = getOtherPerson(survLink, personSurviving);
				personLinkDao.addPersonLink(getPersonLinkFromRecordPair(survOtherPerson, otherPerson, originalLink));
			}
	...
	public void updatePerson(Person person) throws ApplicationException {
		...
		Context.notifyObserver(ObservationEventType.RECORD_UPDATE_EVENT, new Object[]{recordFound, recordUpdated});		
	}
...
//		NotificationEvent event = NotificationEventFactory.createNotificationEvent(EventType.IMPORT_EVENT_TYPE, person);
		...
		for (RecordPairIterator iter = recordPairSource.iterator(); iter.hasNext(); ) {
			...
			if (pair.getMatchOutcome() == RecordPair.MATCH_OUTCOME_LINKED) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Adding review record pair: " + reviewRecordPair);
				}
	...
	private void logPair(RecordPair pair) {
		logRecordIds("N_k", pair.getLeftRecord().getRecordId(), pair.getRightRecord().getRecordId());
	}

	private void logRecordIds(String state, long leftId, long rightId) {
		if (log.isDebugEnabled()) {
			if (leftId < rightId) {
			    log.debug(state + "|" + leftId + "," + rightId);
			} else {
	            log.debug(state + "|" + rightId + "," + leftId);				    
			}
		}
	}
	...
	public void clearLoggedLinks() {
		personLinkDao.clearLoggedLinks();
	}
	...
	private void linkNodesInPair(RecordPair pair, Map<Long,Integer> clusterIdByRecordIdMap, Map<Integer, List<PersonLink>> linksByClusterIdMap, List<PersonLink> links) {
		Record left = personDao.loadRecord(pair.getLeftRecord().getRecordId());
		Record right = personDao.loadRecord(pair.getRightRecord().getRecordId());
