...
//	private LayoutContainer container;
	...
	private List<CustomFieldWeb> customFields;
...
*/
	...
	protected void handleEvent(AppEvent event) {
		if (event.getType() == AppEvents.DataProfileView) {
			...
			customFields = (List<CustomFieldWeb>) event.getData();
	    	...
	    	List<DataProfileAttributeWeb> filterdAttributeList = new ArrayList<DataProfileAttributeWeb>();

	    	// Filter out the not configured custom fields 
			for (DataProfileAttributeWeb attributeWeb : attributeList) {
				 // Info.display("Information", ""+attributeWeb.getAttributeName());
				String attributeName = attributeWeb.getAttributeName();
				if (attributeName.startsWith("custom") ) {
					for (CustomFieldWeb customField : customFields) {
						 if (attributeName.equals(customField.getFieldName())) {
							filterdAttributeList.add(attributeWeb);
							break;
						 }
					}
				} else {
			...
			displayRecords(filterdAttributeList);
